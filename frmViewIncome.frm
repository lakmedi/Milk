...
Begin VB.Form frmViewIncome 
   ...
   End
End
Attribute VB_Name = "frmViewIncome"
...
Private Sub FillGrid()
    ...
    With rsIncome
        ...
        temSql = "SELECT tblIncome.*, tblIncomeCategory.IncomeCategory FROM tblIncome LEFT JOIN tblIncomeCategory ON tblIncome.IncomeCategoryID = tblIncomeCategory.IncomeCategoryID where tblIncome.IncomeDate between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' "
        If IsNumeric(cmbCategory.BoundText) = True Then
            temSql = temSql & " AND tblIncome.IncomeCategoryID = " & Val(cmbCategory.BoundText) & " "
        End If
        
        If IsNumeric(cmbCollectingCenter.BoundText) = True Then
            temSql = temSql & "AND tblIncome.CollectingCenterID = " & Val(cmbCollectingCenter.BoundText) & " "
        End If
        temSql = temSql & " And tblIncome.Deleted = 0 "
        ...
        If .RecordCount > 0 Then
            ...
            For i = 1 To .RecordCount
                gridIncome.TextMatrix(i, 0) = Format(!IncomeDate, "dd MMM yyyy")
                gridIncome.TextMatrix(i, 1) = !IncomeCategory
                gridIncome.TextMatrix(i, 2) = !IncomeComments
                gridIncome.TextMatrix(i, 3) = Format(!IncomeValue, "#,##0.00")
                TemTotal = TemTotal + !IncomeValue
