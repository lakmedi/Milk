...
Begin VB.Form frmApproveAdditionalPayments 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Approve Additional Payments"
   ClientHeight    =   7200
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11175
   ...
   Begin VB.ListBox lstIDs 
      ...
      Width           =   495
   End
   Begin VB.ListBox lstAdditionalPayments 
      Height          =   4380
      Left            =   120
      Style           =   1  'Checkbox
      TabIndex        =   6
   ...
   End
End
Attribute VB_Name = "frmApproveAdditionalPayments"
...
Private Sub btnApprove_Click()
    With rsAP
        ...
        temSql = "Select * from tblAdditionalCommision where AdditionalCommisionID = " & Val(lstIDs.List(lstAdditionalPayments.ListIndex))
...
Private Sub btnApproveSelected_Click()
    Dim i As Integer
    For i = 0 To lstAdditionalPayments.ListCount - 1
        If lstAdditionalPayments.Selected(i) = True Then
        With rsAP
            If .State = 1 Then .Close
            temSql = "Select * from tblAdditionalCommision where AdditionalCommisionID = " & Val(lstIDs.List(i))
            .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
            If .RecordCount > 0 Then
                !Value = !AddedValue
        ...
        End If
    Next
    MsgBox "All selected Commisions Approved"
    ClearDetails
    btnNone_Click
End Sub
...
Private Sub FillLists()
    ...
    With rsAP
        ...
        temSql = "SELECT tblSupplier.Supplier, tblAdditionalCommision.*, tblAdditionalCommision.Deleted " & _
                    ...
                    "WHERE (((tblAdditionalCommision.Deleted) = 0) AND ((tblSupplier.CollectingCenterID)=" & Val(cmbCC.BoundText) & ") AND ((tblAdditionalCommision.CommisionDate) Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "DD MMMM yyyy") & "'))"
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            While .EOF = False
                lstIDs.AddItem !AdditionalCommisionID
                lstAdditionalPayments.AddItem !CommisionDate & " " & !Supplier
                .MoveNext
            Wend
        End If
        .Close
...
Private Sub lstAdditionalPayments_Click()
    Dim i As Integer
    i = lstAdditionalPayments.ListIndex
    Call ClearDetails
    With rsAP
        If .State = 1 Then .Close
        temSql = "Select * from tblAdditionalCommision where AdditionalCommisionID = " & Val(lstIDs.List(i))
