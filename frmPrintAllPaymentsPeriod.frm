...
Begin VB.Form frmPrintAllPaymentsPeriod 
   ...
   Begin VB.ListBox lstBankValue 
      ...
      ItemData        =   "frmPrintAllPaymentsPeriod.frx":0000
      Left            =   4680
      List            =   "frmPrintAllPaymentsPeriod.frx":0002
      MultiSelect     =   2  'Extended
      TabIndex        =   4
      Top             =   1080
      Width           =   2655
   ...
   End
End
Attribute VB_Name = "frmPrintAllPaymentsPeriod"
...
Private Sub ComBankDetails()
     ...
     ComBankAccountName = "Luck Lanka Diaries (pvt) Ltd"
     ComBankAccountNo = "1104013300"
End Sub
...
Private Sub btnExcelCom_Click()
    ...
    With rsReport
        ...
        temSQL = "SELECT  tblSupplier.SupplierID, tblBank.BankCode, tblCity.BankCode, tblSupplier.AccountNo, tblSupplier.AccountHolder, sum(tblSupplierPayments.Value) as TotalValue , max(tblCollectingCenterPaymentSummery.FromDate) as MaxFromDate " & _
                    ...
                    "Where tblCollectingCenterPaymentSummery.FromDate Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' And tblSupplierPayments.GeneratedPaymentMethodID = " & 1 & " And (tblSupplier.BankID = 2 or tblSupplier.BankID = 1 ) " & _
    ...
    End With
            ...
            SaveAsName = App.Path & "\Milk Payment Commercial Bank For Milk Payment From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " To " & Format(dtpFrom.Value, "dd MMMM yyyy") & ".xls"
...
Private Sub btnSampathDetails_Click()
    ...
    With myExcel
        ...
        .SubTopic = "From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " to " & "From " & Format(dtpTo.Value, "dd MMMM yyyy")
        .Path = App.Path
...
'
            temSQL = "SELECT  tblSupplier.SupplierID ,tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')'  AS SupAccount , tblBank.Bank, tblSupplier.SupplierCode, tblSupplier.AccountNo AS Account, sum(tblSupplierPayments.Value) as TotalValue  " & _
                        "FROM ((tblSupplier RIGHT JOIN tblSupplierPayments ON tblSupplier.SupplierID = tblSupplierPayments.SupplierID) RIGHT JOIN (tblCollectingCenter RIGHT JOIN tblCollectingCenterPaymentSummery ON tblCollectingCenter.CollectingCenterID = tblCollectingCenterPaymentSummery.CollectingCenterID) ON tblSupplierPayments.CollectingCenterPaymentSummeryID = tblCollectingCenterPaymentSummery.CollectingCenterPaymentSummeryID) LEFT JOIN tblBank ON tblSupplier.BankID = tblBank.BankID " & _
                        "Where tblCollectingCenterPaymentSummery.FromDate between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "'  And tblSupplierPayments.GeneratedPaymentMethodID = " & 1 & " And tblSupplier.BankID = 1 " & _
                        "GROUP BY tblSupplier.SupplierID, tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')'  , tblBank.Bank, tblSupplier.SupplierCode, tblSupplier.AccountNo " & _
                        ""
        ...
        While .EOF = False
            ...
            myExcel.setValue temRow, 7, !totalValue
    ...
    myExcel.setValue temRow, 2, "Total"
...
Private Sub btnCommercialDetails_Click()
    ...
    With myExcel
        ...
        .SubTopic = "Date " & Format(dtpFrom.Value, "dd MMMM yyyy")
        .Path = App.Path
        .FileName = .Topic & " " & .SubTopic
...
'                    "ORDER BY tblSupplier.Supplier"
              ...
              temSQL = "SELECT  tblSupplier.SupplierID , tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')'  AS SupAccount , tblBank.Bank, tblSupplier.SupplierCode, tblSupplier.AccountNo AS Account, sum(tblSupplierPayments.Value) as TotalValue  " & _
                        ...
                        "Where tblCollectingCenterPaymentSummery.FromDate between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "'  And tblSupplierPayments.GeneratedPaymentMethodID = " & 1 & " And tblSupplier.BankID = 2 " & _
                        "GROUP BY  tblSupplier.SupplierID,  tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')'  , tblBank.Bank, tblSupplier.SupplierCode, tblSupplier.AccountNo " & _
                        "ORDER BY tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')'"
    ...
    myExcel.setValue temRow, 7, Format(temFindVolumePaymentDue, "#,##0.00")
    myExcel.setValue temRow, 2, "Welfare"
...
'                    "ORDER BY tblSupplier.AccountHolder"
        ...
        temSQL = "SELECT tblSupplier.Supplier, tblSupplier.SupplierID, tblSupplier.AccountNo, max(tblSupplierPayments.FromDate) as MaxFromDate, tblBank.BankCode, tblSupplier.AccountHolder, tblSupplier.SupplierID, sum(tblSupplierPayments.Value) as TotalValue , tblSupplierPayments.GeneratedPaymentMethodID, tblBank.Bank, tblCity.City " & _
                    ...
                    "Where tblSupplierPayments.FromDate   between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "'  And tblSupplier.BankID = 1 " & _
...
Private Sub btnPrint_Click()
    ...
    With rsReport
        ...
        temSQL = "SELECT tblSupplier.Supplier + ' (' +  tblSupplier.AccountHolder + ')' AS SupAccount , tblBank.Bank, tblSupplier.AccountNo AS Account, sum(tblSupplierPayments.Value) as TotalValue  " & _
                    "FROM ((tblSupplier RIGHT JOIN tblSupplierPayments ON tblSupplier.SupplierID = tblSupplierPayments.SupplierID) RIGHT JOIN (tblCollectingCenter RIGHT JOIN tblCollectingCenterPaymentSummery ON tblCollectingCenter.CollectingCenterID = tblCollectingCenterPaymentSummery.CollectingCenterID) ON tblSupplierPayments.CollectingCenterPaymentSummeryID = tblCollectingCenterPaymentSummery.CollectingCenterPaymentSummeryID) LEFT JOIN tblBank ON tblSupplier.BankID = tblBank.BankID " & _
                    "Where (((tblCollectingCenterPaymentSummery.FromDate)  between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' ) And ((tblSupplierPayments.GeneratedPaymentMethodID) = " & 2 & ")) " & _
    ...
    With rsSuppliers
        If optAll.Value = True Then
            temSQL = "SELECT tblSupplier.Supplier, tblSupplier.SupplierID,  sum(tblSupplierPayments.Value) as TotalValue ,tblSupplierPayments.GeneratedPaymentMethodID " & _
                        ...
                        "Where (((tblSupplierPayments.FromDate)  between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' )) " & _
        ...
        ElseIf optCom.Value = True Then
            temSQL = "SELECT tblSupplier.Supplier, tblSupplier.SupplierID,  sum(tblSupplierPayments.Value) as TotalValue  ,tblSupplierPayments.GeneratedPaymentMethodID " & _
                        "FROM tblSupplierPayments LEFT JOIN tblSupplier ON tblSupplierPayments.SupplierID = tblSupplier.SupplierID " & _
                        "Where tblSupplierPayments.FromDate   between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & "'  AND tblSupplier.BankID = 2 " & _
                        "GROUP By tblSupplier.Supplier, tblSupplier.SupplierID,  tblSupplierPayments.GeneratedPaymentMethodID " & _
                        "ORDER BY tblSupplier.Supplier"
